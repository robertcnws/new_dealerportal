...
const ModalCustomersSyncComponent = ({ open, handleClose, zohoConfig }) => {
  ...
  const handleSyncCustomers = async () => {
    try {
      const url = `${apiUrl}/z_api/dealerportal-zoho/sync_zoho_customers/`;
      const data = {
      ...
      <Box sx={style}>
        <div className="modal-header" style={{ borderBottom: '1px solid #ddd', marginBottom: 10 }}>
          <Typography id="add-new-quote-modal" variant="h6" component="h5" style={{ marginBottom: 10 }}>
            ...
            Initiate Zoho Customers Sync Manually
          </Typography>
        </div>
        <div className="modal-body">
          ...
          <CustomAlertComponent
            ...
            title={zohoConfig?.connected ? "Warning" : "Error"}
            message={zohoConfig?.connected ? "Please confirm if you want to sync customers. This action cannot be undone." :
              "You are currently not connected to the Zoho API. Please check your API settings and ensure a valid connection before attempting to sync Zoho customers."}
            onClose={handleCloseModal}
          />
          {zohoConfig?.connected && (
          ...
          <div className="modal-footer">
            <Box sx={{ display: 'flex', mt: 2, justifyContent: 'center' }}>
              {zohoConfig?.connected && (
                <Button variant="contained" color="success" onClick={handleSyncCustomers} sx={{ mr: 2 }}>
                  Sync Zoho Customers 
                </Button>
              )}
...
export default ModalCustomersSyncComponent;
