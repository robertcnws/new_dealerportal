...
const ModalAddQuoteComponent = ({ open, handleClose, onSyncComplete, dataEdit, onSyncEdit }) => {
  ...
  }, [dataEdit?.id, fetchQuoteChanges, reset]);
  ...
  const onSubmit = useCallback(
    async (data) => {
      ...
      const markupValue = Number.parseFloat(data.markup);
      const newQuote = {
        ...data,
        markup: Number.isFinite(markupValue) ? markupValue : 0,
        user_id: userId,
      };
      ...
      try {
        const url = dataEdit?.id
          ? `${apiUrl}/dealerportal/quotes/update/${dataEdit.id}/`
          : `${apiUrl}/dealerportal/quotes/create/`;
        ...
        if (response.status === 200) {
          ...
          if (dataEdit?.id && onSyncEdit) {
            await onSyncEdit();
          } else if (onSyncComplete) {
            await onSyncComplete(payload);
          }
    ...
    [dataEdit?.id, handleCloseModal, onSyncComplete, onSyncEdit]
  );
      ...
      <Box sx={style}>
        <div className="modal-header" style={{ borderBottom: '1px solid #ddd', marginBottom: 10 }}>
          <Typography id="add-new-quote-modal" variant="h6" component="h5" style={{ marginBottom: 10 }}>
            {dataEdit ? <i className="bi bi-pencil-square me-2"></i> : <i className="bi bi-plus-circle me-2"></i>}
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <Box className="inputDiv" sx={{ mb: 5 }}>
              <TextField
                ...
                inputProps={{ min: 1 }}
              />
              {errors.markup && <FormHelperText error>{errors.markup.message}</FormHelperText>}
            </Box>
            ...
            <Box className="btnGroup">
              ...
              <Button onClick={handleCloseModal} sx={{ ml: 2, bgcolor: '#F6D3A1', color: 'gray' }} disabled={loading}>
                Cancel
              </Button>
            </Box>
          </form>
